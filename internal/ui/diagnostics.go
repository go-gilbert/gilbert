...
type DiagnosticRenderer interface {
	RenderDiagnostics(diags parsetypes.Diagnostics)
...
func NewJSONDiagnosticRenderer(l *log.Logger) *JSONDiagnosticRenderer {
	return &JSONDiagnosticRenderer{
		log: l,
		fp:  make(filePool, 3),
	}
}

func (*JSONDiagnosticRenderer) SetWriter(io.Writer) {
	// Stub
}

// Reset clears any buffered data
func (r *JSONDiagnosticRenderer) Reset() {
	r.fp = make(filePool, 3)
}

func (r *JSONDiagnosticRenderer) RenderDiagnostics(diags parsetypes.Diagnostics) {
	if len(diags) == 0 {
		return
	}

	l := r.log.Named("diagnostics")
...
type TerminalDiagnosticRenderer struct {
	dst     io.Writer
	fp      filePool
	palette theme.Palette
}

func NewTerminalDiagnosticsRenderer(dst io.Writer, palette theme.Palette) *TerminalDiagnosticRenderer {
	return &TerminalDiagnosticRenderer{
		fp:      make(filePool, 3),
		dst:     dst,
		palette: palette,
	}
...
func (r *TerminalDiagnosticRenderer) SetWriter(w io.Writer) {
	r.dst = w
}

// Reset clears any buffered data
func (r *TerminalDiagnosticRenderer) Reset() {
	r.fp = make(filePool, 3)
}

// RenderDiagnostics renders diagnostics in human-friendly way.
func (r *TerminalDiagnosticRenderer) RenderDiagnostics(diags parsetypes.Diagnostics) {
	if len(diags) == 0 {
		return
	}
...
func (r *TerminalDiagnosticRenderer) renderDiagnostic(diag *parsetypes.Diagnostic) {
